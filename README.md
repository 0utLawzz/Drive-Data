...
- ☁️ **Google Sheets Upload** — Pushes data directly to a Google Sheet using service account credentials. **Note: Duplicate checking has been removed. ALL records are parsed and exported to the sheet and CSV automatically.**
...
## 📊 Pattern and Sheet Structure

The final output (Google Sheets and CSV) follows this sheet structure:
`CLIENT NUMBER` | `CLIENT NAME` | `CASE #` | `CASE NAME` | `TM NO` | `CLASS` | `FILES` | `EXT` | `TM-1` | `TM-48` | `EXAM` | `ACK` | `ACCEPTANCE` | `D-NOTE` | `TM-16` | `TM-50` | `TM-06` | `COMPANY` | `OPPO` | `PUB` | `CERTIFICATE` | `DATE ADDED`

### Pattern Categories

The parser categorizes documents by detecting regex patterns in filenames and mapping them to the columns above.
